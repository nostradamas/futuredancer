...
var tableTitle = "证书、环境信息";
var dataTablesDataUrl = SERVER_PATH + '/admin/aboutus/getEnvironmentLists'; //数据url
var $toObjectDetailUrl = SERVER_PATH+'/admin/aboutus/toEnvironmentDetail';//
var $saveObjectDetailUrl = SERVER_PATH + '/admin/aboutus/environmentSave';
var $deleteObjectUrl = SERVER_PATH + '/admin/aboutus/environmentDelete';
...
$(function() {
	var colsBindData = [
        ...
        {"data" : "type", render:function(data,type,row,meta){
        	var type = data == 1 ?  '证书' : data == 2 ? '环境' : '';
        	return type
        }},
        {"data" : "isDel", render:function(data,type,row,meta){
        	var type = data == 0 ?  '显示' : data == 1 ? '不显示' : '';
        	return type
        }},
        {"data" : "state",render:function(data,type,row,meta){
			var operate = '';

			operate+='<button type="button" class="btn btn-primary btn-sm" onclick="objectEdit(\''+row.eid+'\')">' +
					'<i class="ace-icon fa fa-align-justify fa-lg white"></i> 编辑' +
				'</button>';
			operate+='&nbsp;<button type="button" class="btn btn-primary btn-sm" onclick="objectDelete(\''+row.eid+'\')">' +
